...
- **已完成（M1+M2）**：九阶段全流程可用、Job 系统、厂商能力配置、失效传播、Mock 全链路 + 文本/图像真实适配器。
- **M3**：GPU 集群（ComfyUI Agent 心跳注册）、真实视频/TTS 厂商适配器（Seedance/海螺等）、ShotGroup 首尾帧衔接组、剪辑器 v2.1、单段增强（4K 放大/补帧/对口型）。
- **M4**：对话式剧本交互（LLM patch 流）、声音样本一致性、成品共享反馈闭环、费用余额看板。
