...
- **已完成（M1+M2+M3-lite）**：九阶段全流程可用、Job 系统、厂商能力配置、失效传播、Mock 全链路 + 文本/图像真实适配器、**对话式剧本修改**（多轮对话→patch 预览→确认应用，配置真实 TEXT 厂商后走 LLM）、**ShotGroup 首尾帧衔接组**（长镜头拆段、尾帧自动传递、串行约束、组内失效传播）、**单段增强**（高清放大 1080×1920 / 智能补帧 48fps，本地 FFmpeg 实现）。
