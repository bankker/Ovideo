...
export function buildStoryboardPrompt(
  ...
  return [
    ...
    '拆分策略（硬规则）：同场景无转场的连续剧情合并为一个镜头，单镜头时长按第 7 条规则估算（上限 15 秒，超出必须拆镜）；有转场处必须切镜头。',
    ...
    '7. 时长估算：durationPlannedMs 按台词量估算——有台词镜头 ≈ 台词总字数 × 300 + 2000（毫秒，夹取在 4000~15000）；纯画面/旁白镜头取 5000。配音生成后系统会用实际语音时长精确锁定，此处只需合理初值。',
    '8. 旁白：剧本中说话人写作「旁白」的行，必须输出为 {"isNarrator":true,"text":"…"} 并省略 speaker；【严禁】把「旁白」当作角色写进 tags——它没有形象，不需要设计图，也不该驱动口型。',
